...
const arginfo_raw_build = [_]php.module.zend_internal_arg_info{
...
fn php_raw_build(
    ...
    var inner = php.helpers.newArrayZval(2);
    php.helpers.arrayPushLong(&inner, 7);
    php.helpers.arrayPushStringZ(&inner, "seven");
    ...
    outer.u1.type_info = php.zval.typeInfo(
        php.types.IS_ARRAY,
        php.zval.Z_TYPE_FLAG_REFCOUNTED | php.zval.Z_TYPE_FLAG_COLLECTABLE,
    );

    php.helpers.arraySetLong(&outer, "answer", 42);
    ...
    const str = php.string.dup("raw bytes");
    var sv: php.types.zval = std.mem.zeroes(php.types.zval);
    sv.value.str = str;
    sv.u1.type_info = php.zval.typeInfo(php.types.IS_STRING, php.zval.Z_TYPE_FLAG_REFCOUNTED);
...
const arginfo_raw_find = [_]php.module.zend_internal_arg_info{
    php.module.returnInfo(php.types.MAY_BE_STRING | php.types.MAY_BE_NULL),
    php.module.paramInfo("arr", php.types.MAY_BE_ARRAY),
};
...
fn php_raw_find(
    ...
    const arr = php.params.getArg(execute_data, 1) orelse return php.helpers.returnNull(return_value);
    const found = php.hash.findString(arr.raw(), "data") orelse
...
const arginfo_raw_refcount = [_]php.module.zend_internal_arg_info{
    php.module.returnInfo(php.types.MAY_BE_ARRAY),
...
fn php_raw_refcount(
    ...
    php.helpers.returnArray(return_value, 2);
    php.helpers.arrayPushLong(return_value, before);
    php.helpers.arrayPushLong(return_value, after);
}

const arginfo_raw_manipulate = [_]php.module.zend_internal_arg_info{
    php.module.returnInfo(php.types.MAY_BE_LONG | php.types.MAY_BE_NULL),
    php.module.paramInfo("x", php.types.MAY_BE_LONG),
};
...
fn php_raw_manipulate(
    ...
    if (php.zval.getType(arg.raw()) != php.types.IS_LONG) return php.helpers.returnNull(return_value);

    // Raw union read straight from the zval.
    php.helpers.returnLong(return_value, arg.raw().value.lval * 2);
}

const arginfo_raw_argcount = [_]php.module.zend_internal_arg_info{
...
fn php_raw_argcount(
    ...
    php.helpers.returnLong(return_value, php.params.getArgCount(execute_data));
}
